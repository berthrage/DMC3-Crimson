...
extern func_1DE820_t CreatePlayerCharFunc; // (uint32 character, uint32 id, bool isClone)
...
extern func_280120_t ChangeGunHudAnim; // (byte8 * dest, uint32 side, uint32 index)
...
extern func_1F97F0_t HeadflipAnimation; // (byte8 * actorData, bool noMotion)
extern func_1F94D0_t DevilFluxVFX; // (byte8 * actorData, uint8 index)
extern func_1EAE60_t ActivateDoppelgangerFX; // (byte8 * actorData, uint8 index)
...
extern func_594B0_t InitiateModelFunc;   // (BodyPartData & bodyPartData, byte8 * file, uint32 bodyPart, uint32, byte8 ** motionArchives, byte8 ***
