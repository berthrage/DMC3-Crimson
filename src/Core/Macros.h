...
#define HoboBreak()                 \
    MessageBoxA(0, "break1", 0, 0); \
    MessageBoxA(0, "break2", 0, 0)
...
#define DebugLogFunction(...)                                    \
    if constexpr (debug) {                                       \
        LogFunctionHelper(FUNC_NAME __VA_OPT__(, ) __VA_ARGS__); \
    }

#define LogGUID(name, var) Log(name "%X-%X-%X-%llX", var.Data1, var.Data2, var.Data3, var.Data4)
...
#define OPCODE_CALL 0xE8, 0x00, 0x00, 0x00, 0x00 // call
